...
class SpecializationRepository {
    ...
    public function delete($id){
        try {
            ...
            if($result){
                 return ['status'=>true , 'message'=>'Qualification Delete successfully'];
            }
         } catch (\Throwable $th) {
            //throw $th;
            return ['status' => false, 'message'=>'Qualification Does not Delete'];
         }
    ...
    protected function storeOrUpdate($request, $action)
    {
        try
        {
           $data = $this->model::updateOrCreate(
            ...
            if ($data) {
                $message = $action == "save" ?"Qualification Save Successfully" :"Qualification Update Successfully";
